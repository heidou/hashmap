...
public class FlatBufferBuilder {
	...
	ByteBuffer growByteBuffer(ByteBuffer bb) {
		...
		System.arraycopy(old_buf, 0, new_buf, new_buf_size - old_buf_size,
				old_buf_size);
	...
	// the int length field is aligned to SIZEOF_INT, and the string data
	// follows it
	// directly.
	...
	public void prep(int size, int additional_bytes) {
		...
		int align_size = ((~(bb.array().length - space + additional_bytes)) + 1)
				& (size - 1);
	...
	// Add a scalar to a table at `o` into its vtable, with value `x` and
	// default `d`
